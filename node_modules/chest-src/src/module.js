...
function require(name, callback){
  if (callback)
    // it is a view (delayed require)
    if (m[name]) callback(name, m[name]);
    else new view(name, callback)
  else return m[name];
}

var list = {},
    g_unloaded = 0,
    chest_zmap = ['deps','css','version'];
...
module.prototype = {
  predefine: function(j){
    ...
    if (j && (!this.j || (v = (j.version !== this.j.version)))) {
      if (this.j)
        if (v) m.zipcookie.set('chest_'+this.name, j, chest_zmap);
      else if (this.save)
        m.zipcookie.set('chest_'+this.name, j, chest_zmap);

      this.j = j;

      if (this.j && v) {
        if (j.version !== this.j.version)
          m.zipcookie.set('chest_'+this.name, j, chest_zmap);
      } else if (this.save || j.version !== this.j.version)
        m.zipcookie.set('chest_'+this.name, j, chest_zmap);
      ...
      while (i < c) {
        ...
        if (!(dep = list[depname]))
          dep = new module(depname);

        this.deps[depname] = dep;

        if (dep.delivered) this.unloaded-=1;
        else dep.parents.push(this);
      }
...
window.define = function(name, deps, css, version, run){
  (list[name] || (new module(name, true)))
    .define({ deps: deps, css: css, version: version, run: run });
};
